...
namespace Test.E2E.API
{
    public abstract class E2ETestBase
    {
        ...
        protected readonly WebApplicationFactory<DMFX.NewsAnalysis.API.Startup> _factory;
        protected TestParams _testParams;

        public E2ETestBase(WebApplicationFactory<DMFX.NewsAnalysis.API.Startup> factory)
        {
            this._factory = factory;
        }

        protected DMFX.NewsAnalysis.DTO.LoginResponse Login(string login, string password)
        {
            using (var client = _factory.CreateClient())
            {
                ...
                var dtoResponse = ExtractContentJson<DMFX.NewsAnalysis.DTO.LoginResponse>(respLogin.Result.Content);
