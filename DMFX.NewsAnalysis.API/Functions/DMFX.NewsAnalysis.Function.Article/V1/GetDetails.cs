...
namespace DMFX.NewsAnalysis.Functions.Article.V1
{
    public class GetDetails : FunctionBase
    {
        ...
        public GetDetails(IHttpContextAccessor httpContextAccessor, IArticleDal dalArticle) : base(httpContextAccessor)
        {
            _dalArticle = dalArticle;
        }

        [Authorize]    
        [FunctionName("ArticlesGetDetails")]
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/articles/{id}")] HttpRequest req,
            System.Int64? id,
            ILogger log)
        {
            ...
            try
            {
                var e = _dalArticle.Get(id);
                if (e != null)
                {
                    var dtos = ArticleConvertor.Convert(e, null);
